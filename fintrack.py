...
# Banks / PSPs / fintechs all export transaction statements with different
# header names. These aliases are normalized onto one internal schema so
# almost any CSV/XLSX dump can be dropped in as-is.
COLUMN_ALIASES = {
    "sender_account": [
...
# Second-line defense: if a header doesn't match any alias above exactly,
# this catches it by keyword. Order matters - sender/receiver/amount are
# resolved first since they're required, before looser fields like
# remarks/bank_name that share vaguer keywords.
FALLBACK_KEYWORDS = {
...
def get_txn_path():
    """Get the transaction-file path from the command line, or prompt.

    Works exactly like a drag-and-drop paste: the user can point to any
    file anywhere on their PC, not just files sitting next to the script.
    """
...
def calculate_txn_velocity(records, duration_seconds):
    """Transaction frequency label + rate (records/hour)."""
...
def calculate_risk(records, counterparties, odd_hour_ratio, pass_through_ratio,
    ...
    """Behavioral risk score (0-100) for an account, tuned for mule-account
    and layering detection rather than plain volume."""
    ...
    # Pass-through ratio - the single strongest mule-account signature:
    # money coming in is almost fully moved back out, with little to no
    # accumulation in the account.
    ...
    # Same-day in/out - rapid layering
    ...
    # Round-figure transactions - a common structuring / layering indicator
...
def normalize_header(value):
    ...
    # Drop parentheses/brackets and other stray punctuation banks like to
    # sprinkle into headers (e.g. "Amount (Rs)", "A/C No.") while keeping
    # underscores as the one separator, so aliases still line up.
...
def normalize_columns(df):
    ...
    # Pass 1: exact alias match (handles the vast majority of real exports)
    ...
    # Pass 2: keyword fallback for header spellings the alias list didn't
    # anticipate (e.g. "Payer A/C No", "Beneficiary A/C No", "UTR No").
    # Only fires for a standard field that pass 1 didn't already fill, and
    # only claims columns pass 1 didn't already use.
...
def _read_any_table(path):
    """Best-effort loader: tries Excel and CSV/TSV with several encodings and
    delimiter sniffing so almost any statement export can be read, whatever
    its file extension actually is."""
...
def parse_transactions(path):
    """Load and normalize a financial transaction statement (CSV/XLSX/TSV,
    any bank/PSP export)."""
    ...
    # Drop obviously junk self-transfers with no real counterparty data.
...
def main():
    ...
    if not os.path.exists(txn_path):
        ...
        print()
        print("Usage:")
        print("python fintrack_analyzer.py <statement_file.csv|.xlsx>")
        print()
        sys.exit(1)
    ...
    overall_risk, overall_reasons = calculate_overall_risk(
        total_records, unique_accounts, total_amount, account_results
    )
    ...
    txt_path = os.path.join(REPORT_DIR, f"fintrack_report_{report_file_time}.txt")
    pdf_path = os.path.join(REPORT_DIR, f"fintrack_report_{report_file_time}.pdf")
    xlsx_path = os.path.join(REPORT_DIR, f"fintrack_report_{report_file_time}.xlsx")
    csv_path = os.path.join(REPORT_DIR, f"fintrack_flagged_accounts_{report_file_time}.csv")
    json_path = os.path.join(REPORT_DIR, f"fintrack_report_{report_file_time}.json")
